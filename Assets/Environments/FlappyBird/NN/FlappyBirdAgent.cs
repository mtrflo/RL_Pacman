...
public class FlappyBirdAgent : MonoBehaviour
{
    ...
    void ChooseAction()
    {
        ...
        AddObservation(Mathf.Abs(Mathf.Abs(pipeSpawner.lastPipe.topPoint.position.y) - Mathf.Abs(birdPos.y) ) );// top point distance 
        AddObservation(Mathf.Abs(Mathf.Abs(pipeSpawner.lastPipe.bottomPoint.position.y) - Mathf.Abs(birdPos.y) ) );// bottom point distance 
